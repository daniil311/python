import turtle
import random
import math
import mrrobot


phi = 360 / 7
r = 50

def gotoxy(x, y):
    turtle.penup()
    turtle.goto(x, y)
    turtle.pendown()

def draw_circle(r, color):
    turtle.fillcolor(color)
    turtle.begin_fill()
    turtle.circle(r)
    turtle.end_fill()

def draw_pistol(base_x, base_y):
    #основной круг
    gotoxy(base_x, base_y)
    turtle.circle(80)
    #мушка
    gotoxy(base_x, base_y+160)
    draw_circle(5, 'red')
    #барабан
    for i in range(0, 7):
        phi_rad = phi * i * math.pi / 180.0
        gotoxy(base_x+math.sin(phi_rad) * r, base_y+math.cos(phi_rad) * r + 60)
        draw_circle(22, "white")

def rotate_pistole(base_x, base_y, start):
    for i in range(start, random.randrange(7, 100)):
        phi_rad = phi * i * math.pi / 180.0
        gotoxy(base_x+math.sin(phi_rad) * r, base_y+math.cos(phi_rad) * r + 60)
        draw_circle(22, "brown")
        draw_circle(22, "white")
    gotoxy(base_x+math.sin(phi_rad) * r, base_y+math.cos(phi_rad) * r + 60)
    draw_circle(22, 'brown')
    return i % 7


turtle.hideturtle()
answer = ''
start = 0
turtle.speed(0)
draw_pistol(100,100)
while answer != 'N':
    answer = turtle.textinput("Будем играть?", "Y/N")
    if answer == 'Y':

        start = rotate_pistole(100, 100, start)
        if start == 0:
            gotoxy(-150, 250)
            turtle.write("Вы проиграли и получаете наказание!", font=("Arial", 18, 'normal'))

            z = random.randrange(0, 3)
            if z == 0:
                mrrobot.duble_files('test')
            elif z == 1:
                mrrobot.random_delete('test')
            else:
                gotoxy(-100, -50)
                turtle.write('Вам везет!', font=("Arial", 20, 'normal'))
        else:
            gotoxy(-150, 250)
            turtle.write("Вы выйграли!", font=("Arial", 18, 'normal'))
    elif answer == 'N':
        exit()
    else:
        pass
